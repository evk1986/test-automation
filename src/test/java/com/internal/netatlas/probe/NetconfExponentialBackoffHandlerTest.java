...
public class NetconfExponentialBackoffHandlerTest {
    ...
    @InjectMocks
    private NetconfExponentialBackoffHandler netconfExponentialBackoffHandler;

    @Test
    void testHandle() {
        ProbeJobMessage message = new ProbeJobMessage();
        netconfExponentialBackoffHandler.handle(message);
        verify(netconfExponentialBackoffService).retryWithExponentialBackoff(any());
    }
}